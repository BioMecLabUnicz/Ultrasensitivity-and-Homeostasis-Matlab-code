...
% The script generates the results shown in Figure 3C (or Figure S3E and Figure S3F)  
% by setting Ky=0.4 or 0.1 and by varying the parameters between 10% or (50%) of their nominal values.
...
Ky=0.4; % case 3 with K_y=0.4
%Ky=0.1; % case 3 with K_y=0.1
...
for idx_ny=1:length(v_ny)
    ...
    if idx_ny==1       
        ...
        eval (['load sys_case3','_Ky_',num2str(Ky*100),'_Kx_',num2str(Kx*100),'_nx_',num2str(nx),'_distur_',num2str(perc_dist*100),'_perc100_par_u1']);
    end
    ...
    for idx_sim=1:num_sim
        ...
        if idx_ny==1
            ...
            [~,Zn1]=ode15s(@two_state_system_case_3,tspan,y0,[],my1,k1,gamma1,nx,Kx1,mx1,alpha1,beta1,v_ny(1),Ky1,[],[],k_u,options);               
            y_eq1=Zn1(end,1);
            x_eq1=Zn1(end,2);
            
            [~,Zn2]=ode15s(@two_state_system_case_3,tspan,y0,[],my1,k1,gamma1,nx,Kx1,mx1,alpha1,beta1,v_ny(2),Ky1,[],[],k_u,options);               
            y_eq2=Zn2(end,1);
            x_eq2=Zn2(end,2);
                        
            [~,Zn3]=ode15s(@two_state_system_case_3,tspan,y0,[],my1,k1,gamma1,nx,Kx1,mx1,alpha1,beta1,v_ny(3),Ky1,[],[],k_u,options);               
            y_eq3=Zn3(end,1);
            x_eq3=Zn3(end,2);
            ...
            P=[my1; k1; gamma1;  Ky1; mx1; alpha1; beta1; Kx1;a_ud1];
        ...
        if y_eq<0|| x_eq<0 || y_eq1<0|| x_eq1<0 || y_eq2<0|| x_eq2<0
    ...
    if idx_ny==1
        eval (['save sys_case3','_Ky_',num2str(Ky*100),'_Kx_',num2str(Kx*100),'_nx_',num2str(nx),'_distur_',num2str(perc_dist*100),'_perc100_par_u1   PP']);
    end 
    eval (['save sys_case3','_Ky_',num2str(Ky*100),'_Kx_',num2str(Kx*100),'_ny_',num2str(ny),'_nx_',num2str(nx),'_distur_',num2str(perc_dist*100),'_perc100_par_u1 Z Z_scale ZZ ZZ_scale PP']);
    ...
    [T0,Z0_ny]=ode15s(@two_state_system_case_3,tspan,y0,[],my,k,gamma,nx,Kx,mx,alpha,beta,ny,Ky,tu,du0,k_u,[]);
